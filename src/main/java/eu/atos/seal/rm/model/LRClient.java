...
public class LRClient
{
    ...
    // TODO What to show? And how to show? *** ASK
    public static LRClient getLRClientFrom(LinkRequest linkRequest, int index) 
    {
        ...
        int count = 0;
        for (AttributeType attr: linkRequest.getDatasetA().getAttributes()) {
        ...
        for (AttributeType attr: linkRequest.getDatasetB().getAttributes()) {
        	AttributeClient attrCl = new AttributeClient();
        	attrCl.setIndex(count++);
        	attrCl.setName(attr.getFriendlyName());
        	attrCl.setValue(StringUtils.arrayToCommaDelimitedString(attr.getValues().toArray()));
